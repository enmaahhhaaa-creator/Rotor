...
static RotorConfig BuildMainRotorConfig(void)
{
    ...
    /* 设置主旋翼的基础几何和动力学参数。 */
    rotorConfig.diameter_m = 3.2f;
    rotorConfig.weight_per_blade_lb = 3.97f;
    rotorConfig.num_blades = 5.0f;
    rotorConfig.rel_blade_center = 0.7f;
    rotorConfig.dynamic = 0.7f;
    rotorConfig.delta3 = 0.0f;
    rotorConfig.delta = 1.0f;
    rotorConfig.rpm = 1200.0f;
    ...
    rotorConfig.pitch_b_deg = 14.0f;
    rotorConfig.force_at_pitch_a_lb = 1288.0f;
    rotorConfig.power_at_pitch_0_kw = 72.42913f;
    rotorConfig.power_at_pitch_b_kw = 187.5f;

    /* 直接给出空气动力系数和离散化设置。 */
    rotorConfig.airfoil_lift_coefficient = 8.4f;
    rotorConfig.airfoil_drag_coefficient0 = 0.0119f;
    rotorConfig.airfoil_drag_coefficient1 = 0.0f;
...
static RotorControlInput BuildHoverControl(void)
{
    ...
    /* 构造一个适合悬停工况的控制量。 */
    rotorControl.collective = -0.975f;
